...
class DailyBreakoutStrategy(Strategy):
    ...
    def on_tick(self, timestamp: datetime, price: float):
        ...
        # After first 15-min, check movement
        if not self._first_move_ok:
            move_pct = (self._first_high - self._first_low) / self._first_low * 100
            if move_pct > 1:
                self._first_move_ok = True
            return

        # If breakout not yet entered, check daily breakout
        if self._first_move_ok and not self._entered:
        ...
        if self._entered:
            ...
            if price >= target or price <= stop_loss:
                ...
                self._entered = True  # keep true to prevent re-entry
            return
